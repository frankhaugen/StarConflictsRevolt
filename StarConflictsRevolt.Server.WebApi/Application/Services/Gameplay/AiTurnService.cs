...
public class AiTurnService
{
    ...
    private async Task ProcessSessionAiTurnAsync(GameSessionId sessionId, GameTickMessage tick, CancellationToken stoppingToken)
    {
        ...
        {
            if (stoppingToken.IsCancellationRequested) break;
            await ProcessAiPlayerTurnAsync(sessionAggregate, aiPlayer, tick.TickNumber.Value, stoppingToken);
        }
    ...
    private async Task ProcessAiPlayerTurnAsync(SessionAggregate sessionAggregate, PlayerController aiPlayer, long clientTick, CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            var commands = aiPlayer.GenerateCommands(sessionAggregate.World, clientTick);
            if (commands.Any())
            {
                ...
                {
                    if (stoppingToken.IsCancellationRequested) break;
                    _commandQueue.TryEnqueue(sessionId, cmd);
                    _logger.LogDebug("Queued AI command {CommandType} for player {PlayerId} in session {SessionId}", cmd.GetType().Name, playerId, sessionId);
                }
            }
            else
            {
