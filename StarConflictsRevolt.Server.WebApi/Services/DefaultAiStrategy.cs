    ...
    public List<IGameEvent> GenerateCommands(Guid playerId, World world, ILogger logger)
    {
        ...
        foreach (var fleet in aiFleets)
            if (_random.Next(100) < 30)
            {
                ...
                if (fleet.LocationPlanetId != targetPlanet.Id) commands.Add(new MoveFleetEvent(playerId, fleet.Id, fleet.LocationPlanetId ?? Guid.Empty, targetPlanet.Id));
            }
        ...
        if (enemyFleets.Any() && aiFleets.Any() && _random.Next(100) < 15)
        {
            ...
            if (location != null) commands.Add(new AttackEvent(playerId, attacker.Id, defender.Id, location.Id));
        }
