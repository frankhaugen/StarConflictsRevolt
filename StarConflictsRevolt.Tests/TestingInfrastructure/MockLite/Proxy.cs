...
public class Proxy<T> : DispatchProxy where T : class
{
    ...
    protected override object? Invoke(MethodInfo? target, object?[]? args)
    {
        ...
        _calls.Enqueue(new Invocation(target, args));

        return _rules.TryGetValue(target, out var rule)
            ? rule(args)
            : target.ReturnType.IsValueType
                ? Activator.CreateInstance(target.ReturnType)
                : null;
    }

    // helpers for nested types
    internal void AddRule(MethodInfo m, Func<object?[], object?> r)
    {
        _rules[m] = r;
    }
    ...
    public readonly struct RuleBuilder(MethodInfo method, Proxy<T>? proxy)
    {
        ...
        {
            proxy?.AddRule(method, _ => value);
        }

        /// <summary>Returns a computed value for the arranged call.</summary>
        public void Returns(Func<object?[], object?> factory)
        {
            proxy?.AddRule(method, factory);
        }
    }
}