...
public class TwoPlayerIntegrationTest
{
    ...
    public async Task TwoHumanPlayers_SessionCreationAndJoining_NoAIActions(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                foreach (var delta in deltas) frankReceivedDeltas.Add(delta);
                return Task.CompletedTask;
            });

            mariellHubConnection.On<List<GameObjectUpdate>>("ReceiveUpdates", deltas =>
            {
                foreach (var delta in deltas) mariellReceivedDeltas.Add(delta);
                return Task.CompletedTask;
            });
            ...
            var sessionResponse = await createResponse.Content.ReadFromJsonAsync<SessionResponse>(cancellationToken);
            ...
            var joinSessionResponse = await joinResponse.Content.ReadFromJsonAsync<SessionResponse>(cancellationToken);
            ...
            foreach (var delta in frankReceivedDeltas) await Context.Current.OutputWriter.WriteLineAsync($"[TEST] Frank delta: {delta.Id} - {delta.Type}");

            await Context.Current.OutputWriter.WriteLineAsync($"[TEST] Mariell received {mariellReceivedDeltas.Count} deltas via SignalR");
            foreach (var delta in mariellReceivedDeltas) await Context.Current.OutputWriter.WriteLineAsync($"[TEST] Mariell delta: {delta.Id} - {delta.Type}");
