...
public class FleetFinderView : IView
{
    ...
    private int _selectedFleetIndex = 0;
    ...
    private void DrawActionButtons()
    {
        ...
        if (UIHelper.DrawButton("Select Fleet", startX, buttonY, buttonWidth, buttonHeight))
        {
            SelectCurrentFleet();
        }
        
        if (UIHelper.DrawButton("Move Fleet", startX + buttonWidth + spacing, buttonY, buttonWidth, buttonHeight))
        {
            ShowMoveFleetDialog();
        }
    ...
    private void HandleKeyboardInput()
    {
        ...
        if (Input.IsKeyPressed(KeyboardKey.Up))
        {
            ...
            {
                _selectedFleetIndex--;
                if (_selectedFleetIndex < _scrollOffset)
                {
        ...
        if (Input.IsKeyPressed(KeyboardKey.Down))
        {
            ...
            {
                _selectedFleetIndex++;
                if (_selectedFleetIndex >= _scrollOffset + ItemsPerPage)
                {
                    _scrollOffset = Math.Min(fleets.Count - ItemsPerPage, _scrollOffset + 1);
                }
            }
        }
    ...
    private void ViewFleetDetails()
    {
        ...
        if (_selectedFleetIndex >= 0 && _selectedFleetIndex < fleets.Count)
        {
            _renderContext.GameState.SetFeedback("Fleet details not implemented yet", TimeSpan.FromSeconds(3));
        }
    }
} 