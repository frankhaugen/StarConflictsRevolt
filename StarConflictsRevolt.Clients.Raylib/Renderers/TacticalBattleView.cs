...
public class TacticalBattleView : IView
{
    ...
    private void DrawBattleArea()
    {
        ...
        // Draw grid lines
        for (int x = 0; x <= battleAreaWidth; x += 50)
        {
            Graphics.DrawLine(battleAreaX + x, battleAreaY, battleAreaX + x, battleAreaY + battleAreaHeight, Color.Gray);
        }
        for (int y = 0; y <= battleAreaHeight; y += 50)
        {
            Graphics.DrawLine(battleAreaX, battleAreaY + y, battleAreaX + battleAreaWidth, battleAreaY + y, Color.Gray);
        }
    }
    ...
    private void DrawBattleControls()
    {
        ...
        if (UIHelper.DrawButton("Attack", startX, controlY, buttonWidth, buttonHeight, UIHelper.Colors.Danger))
        {
            _renderContext.GameState.SetFeedback("Attack command sent", TimeSpan.FromSeconds(2));
        }
        
        if (UIHelper.DrawButton("Retreat", startX + buttonWidth + spacing, controlY, buttonWidth, buttonHeight, UIHelper.Colors.Warning))
        {
            _renderContext.GameState.SetFeedback("Retreat command sent", TimeSpan.FromSeconds(2));
        }
        
        if (UIHelper.DrawButton("Formation", startX + (buttonWidth + spacing) * 2, controlY, buttonWidth, buttonHeight))
        {
            _renderContext.GameState.SetFeedback("Formation dialog not implemented yet", TimeSpan.FromSeconds(3));
        }
        
        if (UIHelper.DrawButton("Special", startX + (buttonWidth + spacing) * 3, controlY, buttonWidth, buttonHeight))
        {
            _renderContext.GameState.SetFeedback("Special abilities not implemented yet", TimeSpan.FromSeconds(3));
        }
        
        if (UIHelper.DrawButton("Back to Galaxy", startX + (buttonWidth + spacing) * 4, controlY, buttonWidth, buttonHeight, UIHelper.Colors.Secondary))
        {
    ...
    private void HandleInput()
    {
        ...
        {
            _renderContext.GameState.SetFeedback("Battle paused", TimeSpan.FromSeconds(2));
        }
        
        if (Input.IsKeyPressed(KeyboardKey.A))
        {
            _renderContext.GameState.SetFeedback("Auto-battle enabled", TimeSpan.FromSeconds(2));
        }
    }
} 