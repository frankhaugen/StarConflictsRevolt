...
public class PlanetaryFinderView : IView
{
    ...
    private void DrawPlanetsList()
    {
        ...
        if (displayEnd < planets.Count) UIHelper.DrawText("↓", Window.GetScreenWidth() - 50, startY + panelHeight - 30, UIHelper.FontSizes.Small, Color.Yellow);
    }
    ...
    private void DrawActionButtons()
    {
        ...
        if (UIHelper.DrawButton("Select Planet", startX, buttonY, buttonWidth, buttonHeight)) SelectCurrentPlanet();
        ...
        if (UIHelper.DrawButton("View Details", startX + (buttonWidth + spacing) * 2, buttonY, buttonWidth, buttonHeight)) ViewPlanetDetails();
    ...
    private void HandleKeyboardInput()
    {
        ...
        if (Input.IsKeyPressed(KeyboardKey.Up))
            if (_selectedPlanetIndex > 0)
            {
                ...
                if (_selectedPlanetIndex < _scrollOffset) _scrollOffset = Math.Max(0, _scrollOffset - 1);
            }
        ...
        if (Input.IsKeyPressed(KeyboardKey.Down))
            ...
            {
                _selectedPlanetIndex++;
                if (_selectedPlanetIndex >= _scrollOffset + ItemsPerPage) _scrollOffset = Math.Min(planets.Count - ItemsPerPage, _scrollOffset + 1);
            }
