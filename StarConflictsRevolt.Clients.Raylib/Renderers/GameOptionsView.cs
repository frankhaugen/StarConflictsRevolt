...
public class GameOptionsView : IView
{
    ...
    public void Draw()
    {
        ...
        if (UIHelper.DrawButton("Advanced Options", centerX - 150, startY + optionSpacing * 3, 300, 40, UIHelper.Colors.Secondary)) _showAdvancedOptions = !_showAdvancedOptions;

        if (_showAdvancedOptions) DrawAdvancedOptions(centerX - 150, startY + optionSpacing * 4 + 20);

        // Back button
        if (UIHelper.DrawButton("Back to Menu", centerX - 150, startY + optionSpacing * 5, 300, 40, UIHelper.Colors.Primary)) _renderContext.GameState.NavigateTo(GameView.Menu);
    ...
    private void DrawOption(string label, string value, int x, int y, Action toggleAction)
    {
        ...
        if (UIHelper.DrawButton("Toggle", x + 200, y - 5, 100, 30, UIHelper.Colors.Primary)) toggleAction();
    }
    ...
    {
        if (Input.IsKeyPressed(KeyboardKey.S)) _soundOn = !_soundOn;

        if (Input.IsKeyPressed(KeyboardKey.F)) _fullscreen = !_fullscreen;
